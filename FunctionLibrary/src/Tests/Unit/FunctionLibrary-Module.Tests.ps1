...
Describe 'Module Tests' -Tag Unit {
    Context "Module Tests" {
        ...
        It 'root module FunctionLibrary.psm1 should exist' {
            $PathToModule | Should -Exist
            $? | Should -BeTrue
        } #psm1Exists
        It 'manifest should contain FunctionLibrary.psm1' {
            $PathToManifest |
            Should -FileContentMatchExactly "FunctionLibrary.psm1"
        } #validPSM1
